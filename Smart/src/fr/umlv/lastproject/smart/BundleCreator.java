...
public class BundleCreator {
	...
	public static Bundle createBundle(SmartMapView mapView, boolean missionCreated, 
			boolean trackStarted, GPSTrack gpsTrack, boolean polygonTrackStarted, GPSTrack polygonTrack){
		Bundle b = new Bundle() ;
	...
	public static void saveTrack(Bundle outState,	GPSTrack gpsTrack) {
		...
		for(int i = 0 ; i < count ; i++){
			outState.putSerializable("GPSTRACKPOINT"+i, gpsTrack.getTrackPoints().get(i)) ;
		}
	}

	public static GPSTrack loadTrack(Bundle savedInstanceState,
			SmartMapView mapView, MenuActivity menuActivity, LocationManager lm) {
		...
		GPSTrack.TRACK_MODE mode = TRACK_MODE.valueOf(modeName);
		...
		for(int i = 0 ; i < count ; i ++ ){
			points.add((TrackPoint)savedInstanceState.getSerializable("GPSTRACKPOINT"+i));
		}
		GeometryLayer layer = null ;
		for(GeometryLayer l : mapView.getGeometryOverlays()){
			if(l.getName().equals(name)) layer = l ;
		}
		GPSTrack track = new GPSTrack(mode, name, lm, mapView, GeometryType.LINE, points, layer);
		track.startTrack();
		return track ;
	}

	public static void savePolygonTrack(Bundle outState, 
			GPSTrack gpsTrack) {
		...
		outState.putString("GPSTRACKPOLYGONMODE", gpsTrack.getTrackMode().name());

		outState.putInt("GPSTRACKPOLYGONVALUE", gpsTrack.getTrackMode().getParameter()) ;

		int count = gpsTrack.getTrackPoints().size();
		outState.putInt("GPSTRACKPOLYGONCOUNT", count ) ;

		for(int i = 0 ; i < count ; i++){
			outState.putSerializable("GPSTRACKPOLYGONPOINT"+i, gpsTrack.getTrackPoints().get(i)) ;
		}
	}
	...
	public static GPSTrack loadPolygonTrack(Bundle savedInstanceState,
		...
		boolean started = savedInstanceState.getBoolean("GPSTRACKPOLYGONSTARTED");
		if(!started) return null;
		...
		List<TrackPoint> points = new ArrayList<TrackPoint>() ;
		for(int i = 0 ; i < count ; i ++ ){
			points.add((TrackPoint)savedInstanceState.getSerializable("GPSTRACKPOLYGONPOINT"+i));
		}

		GPSTrack track = new GPSTrack(mode, name, lm, mapView, GeometryType.POLYGON, points, layer);
		track.startTrack();
